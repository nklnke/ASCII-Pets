...
Питомец живёт в прозрачной полноширинной полосе над таскбаром: гуляет (или прыгает), спит, голодает, просит еды, радуется поглаживаниям и иногда издаёт звуки (также иногда какает).
...
| Пёс | `dog` | ходит |
| Ле гушька | `frog` | прыгает по параболе, иногда квакает |

Можно держать до **2 питомцев** одновременно (настройка «Второй питомец» в меню).

Весь арт лежит в `src/renderer/ascii.ts` (`SKINS`). Каждый скин обязан иметь: `walk × 4` + пары `happy / hungry / sleep / jump` + `blink / eat`, все кадры скина — одной (нативной) высоты без пустых строк (в `ascii1`: кот 4, пёс 6, ле гущька 4; в `ascii3`: 6/8/5; проверяется тестом `test/skins.test.mjs`).
